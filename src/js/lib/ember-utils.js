...
Utils.registerHierarchy = function(hierarchy) {
  hierarchy.hierarchyMeta = getMetaFromHierarchy(hierarchy);
};
Utils.addToHierarchy = function(hierarchy, type, classObj, level) {
...
Utils.hasManyWithHierarchy = function(hasManyHierarchy, level, hkey) {
  var meta;
...
Utils.emberDeepEqual = function(src, tar) {
  for(var k in tar) {
    var kObj = src.get(k);
